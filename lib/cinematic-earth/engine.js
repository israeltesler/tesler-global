...
var getSafeCanvasSize = (container, resolutionScale = 1) => {
  ...
  if (container) {
    ...
    if (rect.width > 0) width = Math.min(Math.round(rect.width), viewportWidth);
    if (rect.height > 0)
      height = Math.min(Math.round(rect.height), viewportHeight);
  }
  const pixelRatio =
    Math.min(window.devicePixelRatio || 1, 2) * resolutionScale;
  const maxCssDim = Math.floor(8192 / Math.max(pixelRatio, 1));
  width = Math.min(Math.max(width, 1), maxCssDim);
  height = Math.min(Math.max(height, 1), maxCssDim);
  return { width, height, pixelRatio };
};
var getHeroCanvasSize = (
  resolutionScale = 1,
  container = null,
  lockedSize = null
) => {
  let width;
  let height;
  if (lockedSize?.width > 0 && lockedSize?.height > 0) {
...
var getHeroContainer = (canvas) =>
  canvas?.parentElement?.closest(".cinematic-earth-hero__stage") ??
  canvas?.parentElement ??
  null;
var getStableHeroHeight = () => {
  if (typeof document === "undefined") return 0;
  const raw = getComputedStyle(document.documentElement)
    .getPropertyValue("--hero-stable-height")
    .trim();
...
// Match the original CodePen model exactly: always use its 8K texture set.
var use2k = false;
var TEXTURE_BASE = "/cinematic-earth";
var HERO_TEXTURE_BASE = "/cinematic-earth/hero";
var CONSTANTS = {
  RENDER_TYPE: "webgpu",
  // 'webgpu' or 'webgl'
  EARTH_RADIUS: 10,
  ...
  TEXTURES: {
    ALBEDO: `${TEXTURE_BASE}/8k_earth_daymap.jpg`,
    NIGHT: `${TEXTURE_BASE}/8k_earth_nightmap.jpg`,
    SPECULAR: `${TEXTURE_BASE}/8k_earth_specular_map.jpg`,
    NORMAL: `${TEXTURE_BASE}/8k_earth_normal_map.jpg`,
    CLOUDS: `${TEXTURE_BASE}/8k_earth_clouds.jpg`,
    STARS: `${TEXTURE_BASE}/starmap_8k.jpg`,
    MOON_ALBEDO: `${TEXTURE_BASE}/8k_moon.jpg`,
    MOON_DISPLACEMENT: `${TEXTURE_BASE}/ldem_4.png`,
  },
  GUI: {
    SHOW:
...
async function createEarth(
  ...
  const heroMode = options.heroMode === true;
  const segments = options.segments ?? CONSTANTS.SEGMENTS;
  ...
  if (heroMode) {
    const remainingTexturesPromise = Promise.all([
    ...
    colorMapTex = await loader.loadAsync(CONSTANTS.TEXTURES.ALBEDO);
    [specularMapTex, normalMapTex, cloudsMapTex, nightMapTex] =
      await remainingTexturesPromise;
  } else {
    [colorMapTex, specularMapTex, normalMapTex, cloudsMapTex, nightMapTex] =
      await Promise.all([
  ...
  const geoHigh = new THREE.SphereGeometry(
    CONSTANTS.EARTH_RADIUS,
    segments,
    segments
  );
  const geoMed = new THREE.SphereGeometry(
    CONSTANTS.EARTH_RADIUS,
    Math.max(16, Math.floor(segments / 2)),
    Math.max(16, Math.floor(segments / 2))
  );
  const geoLow = new THREE.SphereGeometry(
    CONSTANTS.EARTH_RADIUS,
  ...
  const sunDirLocal = normalize(
    modelWorldMatrixInverse.mul(vec4(sunDir, 0)).xyz
  );
  ...
  const shadowColorUniform = uniform(
    new THREE.Color(CONSTANTS.GUI.CLOUD_SHADOWS.COLOR)
  );
  ...
  const shadowDist = mix(
    0.1,
    shadowDistUniform,
    smoothstep(0.8, 0, dot(normalize(positionLocal), sunDirLocal))
  );
  ...
  const cloudShadow = mix(
    vec3(1),
    shadowColorUniform,
    shadowOpacity.mul(shadowIntensityUniform)
  );
  const sunDot = dot(normalize(positionWorld), sunDir);
  const nightFade = smoothstep(0.2, -0.2, sunDot);
  const twilightColorUniform = uniform(
    new THREE.Color(CONSTANTS.GUI.ATMOSPHERE.TWILIGHT_COLOR)
  );
  group.userData.twilightColor = twilightColorUniform;
  const darkSideBrightnessUniform = uniform(
    CONSTANTS.GUI.ENVIRONMENT.DARK_SIDE_BRIGHTNESS
  );
  ...
  const twilightTint = mix(
    vec3(1),
    twilightColorUniform,
    twilightFactor.mul(0.5)
  );
  const spec = texture(specularMapTex).r;
  const terrainShadowIntensityUniform = uniform(
    CONSTANTS.GUI.EARTH.TERRAIN_SHADOW_INTENSITY
  );
  const terrainShadowOffsetUniform = uniform(
    CONSTANTS.GUI.EARTH.TERRAIN_SHADOW_OFFSET
  );
  ...
  const pNorm = normalize(
    vTan.mul(nMap.x).add(vBit.mul(nMap.y)).add(surfaceNorm.mul(nMap.z))
  );
  const terrainDot = max(0, dot(pNorm, sunDirLocal));
  const sunProj = sunDirLocal.sub(
    surfaceNorm.mul(dot(sunDirLocal, surfaceNorm))
  );
  const sunT = normalize(sunProj.add(vec3(1e-6)));
  const offsetPos = normalize(
    positionLocal.add(sunT.mul(terrainShadowOffsetUniform))
  );
  const offsetUv = equirectUV(offsetPos);
  const offsetNMap = texture(normalMapTex, offsetUv).xyz.mul(2).sub(1);
  const pNormOffset = normalize(
    vTan
      .mul(offsetNMap.x)
      .add(vBit.mul(offsetNMap.y))
      .add(surfaceNorm.mul(offsetNMap.z))
  );
  ...
  const selfShadowFactor = smoothstep(0, 0.3, occlusion)
    .mul(landMask)
    .mul(terrainShadowIntensityUniform)
    .mul(daylightMask);
  const terrainShadowColor = mix(
    vec3(1),
    vec3(0.1, 0.15, 0.2),
    selfShadowFactor
  );
  const fragmentToMoon = sub(moonPosUniform, positionWorld);
  const _distToMoon = length(fragmentToMoon);
  const dirFragmentToMoon = normalize(fragmentToMoon);
  const thetaMoon = acos(
    max(float(-1), min(float(1), dot(dirFragmentToMoon, sunDir)))
  );
  ...
  const eclipseDimmer = mix(
    vec3(1),
    vec3(0.015, 0.02, 0.025),
    moonEclipseShadow
  );
  earthMaterial.colorNode = texture(colorMapTex)
    .mul(cloudShadow)
    .mul(twilightTint)
    .mul(terrainShadowColor)
    .mul(eclipseDimmer);
  ...
  earthMaterial.specularIntensityNode = mix(
    float(1),
    float(0),
    moonEclipseShadow
  );
  earthMaterial.iorNode = mix(float(1.5), float(1), moonEclipseShadow);
  const bumpScaleUniform = uniform(
    vec2(CONSTANTS.GUI.EARTH.BUMP_SCALE, CONSTANTS.GUI.EARTH.BUMP_SCALE)
  );
  group.userData.bumpScale = bumpScaleUniform;
  const bumpFade = smoothstep(-0.15, 0.15, sunDot);
  earthMaterial.normalNode = normalMap(
    texture(normalMapTex),
    bumpScaleUniform.mul(bumpFade)
  );
  const nightLights = texture(nightMapTex)
    .mul(nightFade)
    .mul(cityLightsUniform);
  const darkSideAmbient = texture(colorMapTex)
    .mul(nightFade)
    .mul(darkSideBrightnessUniform)
    .mul(0.5);
  ...
  const cloudsGeoHigh = new THREE.SphereGeometry(
    CONSTANTS.EARTH_RADIUS + 0.05,
    CONSTANTS.SEGMENTS,
    CONSTANTS.SEGMENTS
  );
  const cloudsGeoMed = new THREE.SphereGeometry(
    CONSTANTS.EARTH_RADIUS + 0.05,
    Math.max(16, Math.floor(CONSTANTS.SEGMENTS / 2)),
    Math.max(16, Math.floor(CONSTANTS.SEGMENTS / 2))
  );
  const cloudsGeoLow = new THREE.SphereGeometry(
    CONSTANTS.EARTH_RADIUS + 0.05,
    Math.max(8, Math.floor(CONSTANTS.SEGMENTS / 4)),
    Math.max(8, Math.floor(CONSTANTS.SEGMENTS / 4))
  );
  const cloudsMaterial = new MeshPhysicalNodeMaterial();
  const finalCloudOpacity = texture(cloudsMapTex).r;
  const baseDarkSideScatter = mix(
    vec3(5e-3, 7e-3, 0.01),
    vec3(0.05, 0.06, 0.08),
    nightFade
  );
  const darkSideScatter = baseDarkSideScatter
    .mul(darkSideBrightnessUniform)
    .mul(20);
  ...
  cloudsMaterial.specularIntensityNode = mix(
    float(1),
    float(0),
    moonEclipseShadow
  );
  cloudsMaterial.iorNode = mix(float(1.5), float(1), moonEclipseShadow);
  cloudsMaterial.normalNode = bumpMap(
    texture(cloudsMapTex),
    float(0.02).mul(bumpFade)
  );
  ...
  const atmosModeUniform = uniform(
    CONSTANTS.GUI.ATMOSPHERE.MODE === "Scattering" ? 0 : 1
  );
  ...
  const innerFinalAirglow = innerFinalScattering
    .mul(0.5)
    .add(
      airglowColorUniform.mul(innerOpticalDepth).mul(0.5).mul(intensityPhase)
    );
  innerAtmosMaterial.colorNode = mix(
...
async function createMoon(textureLoader, maxAnisotropy = 1) {
  ...
  const displacementMap = await textureLoader.loadAsync(
    CONSTANTS.TEXTURES.MOON_DISPLACEMENT
  );
...
function updateLensFlare(
  ...
  if (projectionLength > 0 && projectionLength < sunDist) {
    ...
    const camUp = new THREE3.Vector3(0, 1, 0).applyQuaternion(
      camera.quaternion
    );
    const camRight = new THREE3.Vector3(1, 0, 0).applyQuaternion(
      camera.quaternion
    );
    ...
    if (sunY >= radius) {
      ...
      if (isInside) {
        ...
        const innerRange =
          radius * (anamorphicSettings?.innerFade ?? 0.02) * adaptiveFactor;
        ...
        const outerRange =
          radius * (anamorphicSettings?.outerFade ?? 0.4) * adaptiveFactor;
  ...
  if (moonMesh && moonSettings && moonSettings.enabled) {
    ...
    const _moonDist = moonMesh.position.distanceTo(camera.position);
    if (moonProjectionLength > 0 && moonProjectionLength < sunDist) {
      ...
      if (moonSunY < moonRadius) {
        const moonEdgeX = Math.sqrt(
          Math.max(0, moonRadius * moonRadius - moonSunY * moonSunY)
        );
        ...
        if (isMoonInside) {
          ...
          const innerRange =
            moonRadius *
            (anamorphicSettings?.innerFade ?? 0.02) *
            adaptiveMoonFactor;
          if (innerDist < innerRange) {
            const boost = 1 - innerDist / innerRange;
          ...
          const outerRange =
            moonRadius *
            ((anamorphicSettings?.outerFade ?? 0.4) * 1.25) *
            adaptiveMoonFactor;
          if (outerDist < outerRange) {
            const boost = 1 - outerDist / outerRange;
            anamorphicOcclusionFactor = Math.max(
...
function buildGui(gui, options) {
  ...
  sunFolder
    .add(sunSettings, "intensity", 0, 10)
    .name("Intensity")
    .onChange((v) => {
  ...
  sunFolder
    .add(sunSettings, "angle", 0, Math.PI * 2)
    .name("Manual Angle")
    .listen();
  ...
  moonFolder
    .add(moonSettings, "displacementScale", 0, 0.2)
    .name("Displacement")
    .onChange((v) => {
  ...
  moonFolder
    .add(moonSettings, "illumination", 0, 1)
    .name("Illumination")
    .onChange((v) => {
      const target = moonMesh;
  ...
  skyFolder.add(scene, "backgroundIntensity", 0, 5).name("Skybox Intensity");
  skyFolder
    .add(scene.backgroundRotation, "y", 0, Math.PI * 2)
    .step(0.01)
    .name("Yaw / Longitude (Y)");
  skyFolder
    .add(scene.backgroundRotation, "x", 0, Math.PI * 2)
    .step(0.01)
    .name("Pitch / Latitude (X)");
  skyFolder
    .add(scene.backgroundRotation, "z", 0, Math.PI * 2)
    .step(0.01)
    .name("Roll (Z)");
  if (backgroundStarsSettings && backgroundStars) {
    ...
    skyFolder
      .add(backgroundStarsSettings, "count", 0, 2e4, 100)
  ...
  earthGroup
    .add(earthSettings, "rotationSpeed", 0, 0.01)
    .name("Rotation Speed")
    .step(1e-4);
  ...
  atmosFolder
    .add(earth.userData.atmosDensity, "value", 0.1, 100)
    .name("Density");
  atmosFolder
    .add(earth.userData.rayleighIntensity, "value", 0, 5)
    .step(0.1)
    .name("Rayleigh Intensity");
  atmosFolder
    .add(earth.userData.darkSideBrightness, "value", 0, 0.5)
    .step(1e-3)
    .name("Overall Dark Side");
  atmosFolder
    .add(earth.userData.cityLights, "value", 0, 20)
    .step(0.1)
    .name("City Lights");
  ...
  oceanFolder
    .add(earth.userData.waterRoughness, "value", 0, 1)
    .name("Water Roughness");
  oceanFolder
    .add(earth.userData.waterMetalness, "value", 0, 1)
  ...
  terrainFolder
    .add(terrainSettings, "bumpScale", 0, 10)
    .name("Bump Map Scale")
    .onChange((v) => {
      earth.userData.bumpScale.value.set(v, v);
    });
  terrainFolder
    .add(earth.userData.terrainShadowIntensity, "value", 0, 5)
    .name("Self-Shadow Intensity");
  terrainFolder
    .add(earth.userData.terrainShadowOffset, "value", 1e-4, 0.01)
    .name("Self-Shadow Offset");
  if (satelliteSettings) {
    const satGroup = earthGroup.addFolder("Satellites");
    satGroup
      .add(satelliteSettings, "enabled")
    ...
    satGroup
      .add(satelliteSettings, "speedScale", 0, 5)
      .step(0.1)
      .name("Orbit Speed");
  }
  ...
  ccFolder
    .add(cgSettings, "contrast", 0.5, 2)
    .name("Contrast")
    .onChange((v) => {
      cgUniforms.contrast.value = v;
    });
  ccFolder
    .add(cgSettings, "saturation", 0, 2)
    .name("Saturation")
    .onChange((v) => {
      cgUniforms.saturation.value = v;
    });
  ccFolder
    .add(cgSettings, "blackLevel", 0, 0.5)
    .name("Black Level")
    .onChange((v) => {
      cgUniforms.blackLevel.value = v;
    });
  ccFolder
    .add(cgSettings, "blueGreenBoost", 0, 1)
  ...
  anaFolder
    .add(anamorphicSettings, "innerFade", 1e-3, 1)
    .name("Inner Fade")
    .step(1e-3);
  anaFolder
    .add(anamorphicSettings, "outerFade", 0.01, 1)
    .name("Outer Fade")
    .step(0.01);
  ...
  caFolder
    .add(caSettings, "strength", 0, 5)
    .name("Strength")
    .onChange((v) => {
      if (caSettings.enabled) caUniforms.strength.value = v;
    });
  caFolder
    .add(caSettings, "scale", 0.5, 2)
    .name("Scale")
    .onChange((v) => {
  ...
  filmFolder
    .add(filmSettings, "intensity", 0, 1)
    .name("Intensity")
    .onChange((v) => {
  ...
  vignetteFolder
    .add(vignetteSettings, "darkness", 0, 5)
    .step(0.1)
    .name("Darkness")
    .onChange((v) => {
  ...
  vignetteFolder
    .add(vignetteSettings, "offset", 0, 2)
    .step(0.01)
    .name("Offset")
    .onChange((v) => {
  ...
  debugFolder
    .add(options.renderSettings, "resolutionScale", 0.1, 2)
    .step(0.01)
  ...
  const debugActions = {
    ...
    takeScreenshot: () => {
      ...
      const finalWidth = Math.round(
        parentWidth * Math.min(dpr, 2) * scaleFactor
      );
...
var BackgroundStars = class {
  ...
  #build(coolColor, warmColor) {
    ...
    return new THREE4.InstancedMesh(
...
var Engine = class {
  constructor(canvas, options = {}) {
    this.heroMode = options.heroMode === true;
    this.animationId = 0;
    ...
    this.heroHasUserInteraction = false;
    this._heroLockedSize = null;
    this._heroRelayoutNeeded = false;
    this._heroScrollProgress = 1;
    this._heroInteractionEnabled = true;
    ...
    this.handleResize = () => {
      ...
      const scale = this.renderSettings
        ...
        : 1;
      const heroContainer = this.heroMode
        ? getHeroContainer(this.canvas)
        : null;
      const lockHeroViewport =
        this.heroMode && isMobileOrTablet() && this._heroLockedSize;
      const { width, height, pixelRatio } = this.heroMode
        ? getHeroCanvasSize(
            scale,
            heroContainer,
            lockHeroViewport ? this._heroLockedSize : null
          )
        : getSafeCanvasSize(this.canvas.parentElement, scale);
      if (this.heroMode) {
        if (isMobileOrTablet()) {
          if (!this._heroLockedSize && width > 0 && height > 0) {
            this._heroLockedSize = { width, height };
          }

          if (this._heroRelayoutNeeded) {
            const { aspect, fov, minDistance, maxDistance } =
              getHeroCameraDistances(width / height);
      ...
      if (this.backgroundStars) {
        const pPU =
          height /
          (2 * Math.tan(THREE5.MathUtils.degToRad(this.camera.fov / 2)));
    ...
    this.animate = () => {
      ...
      if (
        this.satelliteSettings.enabled &&
  ...
  async init(onProgress, initOptions = {}) {
    if (onProgress) onProgress("Initializing WebGPU Renderer");
    const { WebGPURenderer } = await import("three/webgpu");
    if (this.isDisposed) return;
    let useWebGL = CONSTANTS.RENDER_TYPE === "webgl";
    if (!useWebGL && typeof navigator !== "undefined" && !navigator.gpu) {
      useWebGL = true;
    }
    ...
    this.renderer = createRenderer(useWebGL);
    try {
    ...
    const controlSurface =
      this.heroMode && this.canvas.parentElement
    ...
    if (this.heroMode) {
      ...
      const heroEventSurface =
        controlSurface.closest(".cinematic-earth-hero__stage") ??
        controlSurface;
      this._heroPointer = null;
      this._heroMoveListenerActive = false;

      const resumeHeroAutoRotate = () => {
        this.controls.autoRotate = true;
        this.controls.autoRotateSpeed = CONSTANTS.GUI.CAMERA.AUTO_ROTATE_SPEED;
      };
      const setHeroCursor = (cursor) => {
        controlSurface.style.cursor = cursor;
        if (heroEventSurface !== controlSurface) {
          heroEventSurface.style.cursor = cursor;
        }
      };
      const detachHeroMoveListener = () => {
        if (!this._heroMoveListenerActive) return;
        window.removeEventListener("pointermove", onPointerMove);
        this._heroMoveListenerActive = false;
      };
      const clearHeroPointer = ({ resumeAutoRotate = true } = {}) => {
        this._heroPointer = null;
        detachHeroMoveListener();
        this.setHeroTouchScrollLock(false);
        this.controls.enabled = false;
        setHeroCursor("");
        if (resumeAutoRotate) {
      ...
      const onPointerMove = (event) => {
        const pointer = this._heroPointer;
        if (!pointer || event.pointerId !== pointer.id) return;

        if (pointer.mode === "mouse" || pointer.mode === "rotate") {
          ...
          if (deltaX !== 0 || deltaY !== 0) {
            this.heroHasUserInteraction = true;
            this.controls.autoRotate = false;
            this.rotateHeroCameraByDelta(deltaX, deltaY);
          }
          return;
        }
        ...
        const travel = Math.max(Math.abs(totalDx), Math.abs(totalDy));
        if (pointer.mode === "pending") {
          if (travel < HERO_TOUCH.GESTURE_THRESHOLD) return;
          if (
            Math.abs(totalDy) >
            Math.abs(totalDx) * HERO_TOUCH.SCROLL_DOMINANCE
          ) {
            clearHeroPointer({ resumeAutoRotate: false });
            return;
          }
          pointer.mode = "rotate";
          this.heroHasUserInteraction = true;
          this.controls.autoRotate = false;
          this.setHeroTouchScrollLock(true);
      ...
      const attachHeroMoveListener = () => {
        if (this._heroMoveListenerActive) return;
        window.addEventListener("pointermove", onPointerMove, {
          passive: false,
        });
        this._heroMoveListenerActive = true;
      };
      const onPointerDown = (event) => {
        if (event.button !== 0) return;
        if (!this._heroInteractionEnabled) return;
        ...
        this._heroPointer = {
          ...
          lastX: event.clientX,
          lastY: event.clientY,
          mode: event.pointerType === "mouse" ? "mouse" : "pending",
        };
        this.controls.autoRotate = false;
        if (event.pointerType === "mouse") {
          setHeroCursor("grabbing");
        }
        attachHeroMoveListener();
      };
      const onPointerEnd = (event) => {
        if (this._heroPointer && event.pointerId !== this._heroPointer.id)
          return;
        const didInteract =
          this._heroPointer?.mode === "mouse" ||
          this._heroPointer?.mode === "rotate";
        clearHeroPointer();
        if (didInteract) {
          this.heroHasUserInteraction = true;
          this.controls.saveState();
      ...
      heroEventSurface.addEventListener("pointerdown", onPointerDown);
      window.addEventListener("pointerup", onPointerEnd);
      window.addEventListener("pointercancel", onPointerEnd);
      this._heroInteractionCleanup = () => {
        clearHeroPointer();
        heroEventSurface.removeEventListener("pointerdown", onPointerDown);
    ...
    this.controls.addEventListener("end", () => {
      if (this.heroMode) {
        if (this.heroHasUserInteraction) {
          this.controls.saveState();
        }
        this.controls.autoRotate = true;
        this.controls.autoRotateSpeed = CONSTANTS.GUI.CAMERA.AUTO_ROTATE_SPEED;
    ...
    this.stats = null;
    if (!this.heroMode && CONSTANTS.GUI.DEBUG.STATS) {
    ...
    this.moonMesh = null;
    if (CONSTANTS.GUI.MOON.ENABLED && !this.heroMode) {
    ...
    if (this.heroMode) {
      this.scene.background = new THREE5.Color(0x0a0a0a);
    } else {
      const starsTexture = await this.textureLoader.loadAsync(
        CONSTANTS.TEXTURES.STARS
      );
      if (this.isDisposed) return;
      starsTexture.mapping = THREE5.EquirectangularReflectionMapping;
      starsTexture.colorSpace = THREE5.SRGBColorSpace;
      this.scene.background = starsTexture;
      this.scene.backgroundRotation.order = "YXZ";
      this.scene.backgroundIntensity =
        CONSTANTS.GUI.ENVIRONMENT.SKYBOX_INTENSITY;
      this.scene.backgroundRotation.y =
        CONSTANTS.GUI.ENVIRONMENT.SKYBOX_AZIMUTH;
      this.scene.backgroundRotation.x = CONSTANTS.GUI.ENVIRONMENT.SKYBOX_PITCH;
      this.scene.backgroundRotation.z = CONSTANTS.GUI.ENVIRONMENT.SKYBOX_ROLL;
    }
    ...
    const earth = await createEarth(
      ...
      this.moonPosUniform,
      maxAnisotropy,
      { heroMode: this.heroMode, segments: CONSTANTS.SEGMENTS }
    );
    if (this.isDisposed) return;
    this.root.add(earth);
    ...
    if (this.heroMode) {
      this.renderPipeline.outputNode = scenePass.toneMapping(
      ...
      this.flareIntensityUniform = uniform3(0);
      this.anamorphicSettings = {
        enabled: false,
        intensity: 0,
        thickness: 0,
      ...
      this.anamorphicSizeUniform = uniform3(0);
      this.anamorphicThicknessUniform = uniform3(0);
      this.anamorphicColorUniform = uniform3(new THREE5.Color(0xffffff));
      this.flareSettings = { enabled: false, intensity: 0 };
      this.caSettings = { enabled: false, strength: 0, scale: 0 };
      this.filmSettings = { enabled: false, intensity: 0 };
      this.vignetteSettings = { enabled: false, darkness: 0, offset: 0 };
    } else {
    this.bloomSettings = {
    ...
    const flareColorModified = this.sunColorUniform
      .mul(vec33(1.2, 1.2, 1.2))
      .mul(lF);
    const colorFlare = mul2(
      ccWgsl({
    ...
    const colorAnamorphic = mul2(
      this.anamorphicColorUniform.mul(aF),
      this.anamorphicIntensityUniform
    );
    ...
    // The original post-processing chain contains WGSL-only functions. Keep it
    // unchanged on WebGPU; use the unmodified scene on WebGL so Safari and
    // non-WebGPU phones still render the same 8K earth instead of a black frame.
    this.renderPipeline.outputNode = this.usingWebGL
      ? scenePass.toneMapping(THREE5.ACESFilmicToneMapping)
      : smaa(finalNode);
    this.flareSettings = {
    ...
    this.citiesSettings = {
      enabled:
        CONSTANTS.GUI.CITIES?.ENABLED !== void 0
          ? CONSTANTS.GUI.CITIES.ENABLED
          : false,
    };
    this.gui = null;
    const debugSettings = { stats: CONSTANTS.GUI.DEBUG.STATS };
    if (!this.heroMode && CONSTANTS.GUI.SHOW) {
    ...
    this.handleResize();
    if (this.heroMode) {
      this._heroOrientationHandler = () => {
        this._heroRelayoutNeeded = true;
        this._heroLockedSize = null;
    ...
    if (this.heroMode) {
      initOptions.onHeroReady?.();
    }
    if (onProgress) onProgress("Compiling Shaders (Warmup)");
    if (this.renderer && !this.isDisposed && !this.heroMode) {
  ...
  initSatellites() {
    ...
    this.satColorUniform = uniform3(
      new THREE5.Color(this.satelliteSettings.color)
    );
  ...
  setHeroScrollProgress(progress) {
    ...
    this._heroScrollProgress = THREE5.MathUtils.clamp(
  ...
  setEarthVisible(visible) {
    if (!this.heroMode || !this.earthGroup) return;
    this.earthGroup.visible = visible;
  }
  _applyHeroScrollCamera() {
    if (!this.heroMode || !this.camera || !this.controls) return;
    const { defaultDistance, minDistance, maxDistance, targetY } =
      getHeroCameraDistances(this.camera.aspect || 1);
    const progress = this._heroScrollProgress;
    const eased = progress * progress * (3 - 2 * progress);
    const introDistance = defaultDistance * 2.4;
    const offset = this.camera.position.clone().sub(this.controls.target);
    const spherical = new THREE5.Spherical().setFromVector3(offset);

    if (!Number.isFinite(spherical.theta) || !Number.isFinite(spherical.phi)) {
      spherical.theta = 0;
      spherical.phi = Math.PI / 2;
    }

    spherical.radius = THREE5.MathUtils.lerp(
      introDistance,
      defaultDistance,
      eased
    );
    spherical.makeSafe();
    this.controls.target.set(0, THREE5.MathUtils.lerp(0, targetY, eased), 0);
    offset.setFromSpherical(spherical);
    this.camera.position.copy(this.controls.target).add(offset);
    this.controls.minDistance = minDistance;
    this.controls.maxDistance = Math.max(maxDistance, introDistance * 1.02);
    this.controls.update();
  }
  rotateHeroCameraByDelta(deltaX, deltaY) {
    if (!this.camera || !this.controls) return;
    const target = this.controls.target;
    const offset = this.camera.position.clone().sub(target);
    const spherical = new THREE5.Spherical().setFromVector3(offset);
    spherical.theta -= deltaX * HERO_TOUCH.ROTATE_SPEED;
    spherical.phi -= deltaY * HERO_TOUCH.ROTATE_SPEED;
    spherical.phi = THREE5.MathUtils.clamp(
      spherical.phi,
  ...
  setHeroTouchScrollLock(locked) {
    const surface = this.canvas?.parentElement;
    const stage = surface?.closest(".cinematic-earth-hero__stage");
    const surfaces = [surface, stage].filter(Boolean);
    for (const node of surfaces) {
      if (locked) {
        node.setAttribute("data-lenis-prevent-touch", "");
        if (node.classList.contains("cinematic-earth-canvas")) {
          node.classList.add("cinematic-earth-canvas--rotating");
          node.style.touchAction = "none";
        }
      } else {
        node.removeAttribute("data-lenis-prevent-touch");
        if (node.classList.contains("cinematic-earth-canvas")) {
          node.classList.remove("cinematic-earth-canvas--rotating");
  ...
  dispose() {
    ...
    if (this._heroOrientationHandler) {
      window.removeEventListener(
        "orientationchange",
        this._heroOrientationHandler
      );
      this._heroOrientationHandler = null;
    }
    this._heroInteractionCleanup?.();
    this._heroInteractionCleanup = null;
    if (this.gui) {
      this.gui.destroy();
    }
  ...
  updateProjectedLocations() {
    ...
    const width = this.canvas.parentElement
      ...
      : window.innerWidth;
    const height = this.canvas.parentElement
      ? this.canvas.parentElement.clientHeight
      : window.innerHeight;
    const dirToCamera = this.camera.position
      .clone()
      .sub(earthCenter)
      .normalize();
...
function initVanillaApp() {
  ...
  const updateTimes = () => {
    ...
    for (const loc of CINEMATIC_LOCATIONS) {
      ...
      const local = new Date(utc + 36e5 * offset);
...
if (typeof document !== "undefined") {
  ...
  if (
    ...
    document.addEventListener("DOMContentLoaded", bootVanillaApp);
  }
}

function configureHeroDefaults() {
  CONSTANTS.GUI.SHOW = false;
  CONSTANTS.GUI.DEBUG.STATS = false;
  CONSTANTS.GUI.CAMERA.AUTO_ROTATE = true;
  CONSTANTS.GUI.CAMERA.AUTO_ROTATE_SPEED = 1.35;
  CONSTANTS.GUI.MOON.ENABLED = false;
  CONSTANTS.SEGMENTS = isMobileOrTablet() ? 48 : 96;
  CONSTANTS.GUI.DEBUG.RESOLUTION_SCALE = getInitialResolutionScale();
  CONSTANTS.GUI.BLOOM.ENABLED = false;
  CONSTANTS.GUI.CHROMATIC_ABERRATION.ENABLED = false;
  CONSTANTS.GUI.VIGNETTE.ENABLED = false;
  CONSTANTS.GUI.FILM_GRAIN.ENABLED = false;
  CONSTANTS.GUI.LENS_FLARE.ENABLED = false;
  CONSTANTS.GUI.ANAMORPHIC.ENABLED = false;
  CONSTANTS.TEXTURES = {
    ALBEDO: `${HERO_TEXTURE_BASE}/8k_earth_daymap.jpg`,
    NIGHT: `${HERO_TEXTURE_BASE}/8k_earth_nightmap.jpg`,
    SPECULAR: `${HERO_TEXTURE_BASE}/8k_earth_specular_map.jpg`,
    NORMAL: `${HERO_TEXTURE_BASE}/8k_earth_normal_map.jpg`,
    CLOUDS: `${HERO_TEXTURE_BASE}/8k_earth_clouds.jpg`,
    STARS: `${TEXTURE_BASE}/starmap_8k.jpg`,
    MOON_ALBEDO: `${TEXTURE_BASE}/8k_moon.jpg`,
    MOON_DISPLACEMENT: `${TEXTURE_BASE}/ldem_4.png`,
  };
...
async function waitForCanvasSize(canvas, attempts = 120) {
  const container = getHeroContainer(canvas);
  for (let index = 0; index < attempts; index += 1) {
    const { width, height } = getHeroCanvasSize(1, container);
    if (width > 0 && height > 0) {
      return { width, height };
    }
    await new Promise((resolve) => requestAnimationFrame(resolve));
  }
  return getHeroCanvasSize(1, container);
}
...
var HERO_TOUCH = {
  GESTURE_THRESHOLD: 14,
  ROTATE_SPEED: 0.0035,
  SCROLL_DOMINANCE: 1.35,
};

function getHeroCameraDistances(aspect) {
  const fov = CONSTANTS.GUI.CAMERA.FOV;
  const vFov = THREE5.MathUtils.degToRad(fov);
  const hFov = 2 * Math.atan(Math.tan(vFov / 2) * aspect);
  const limitingFov = Math.min(vFov, hFov);
  const baseDistance = CONSTANTS.EARTH_RADIUS / Math.sin(limitingFov / 2);
  return {
    aspect,
    fov,
    minDistance: baseDistance / HERO_CAMERA.FILL_NEAR,
    defaultDistance: baseDistance / HERO_CAMERA.FILL_DEFAULT,
    maxDistance: baseDistance / HERO_CAMERA.FILL_FAR,
    targetY: HERO_CAMERA.TARGET_Y,
    positionY: HERO_CAMERA.POSITION_Y,
  };
}

function applyHeroCamera(engine, viewport) {
  ...
  engine.camera.position.set(0, positionY, defaultDistance);
  engine.camera.lookAt(0, targetY, 0);
  engine.camera.updateProjectionMatrix();
  engine.controls.minDistance = minDistance;
  engine.controls.maxDistance = maxDistance;
  engine.controls.target.set(0, targetY, 0);
  engine.controls.update();
}

async function createHeroEngine(canvas, { onProgress, onReady } = {}) {
  configureHeroDefaults();
  const viewport = await waitForCanvasSize(canvas);
  const engine = new Engine(canvas, { heroMode: true });
  await engine.init(onProgress, { onHeroReady: onReady });
  applyHeroCamera(engine, viewport);
  engine.handleResize();
  if (isMobileOrTablet()) {
    const heroContainer = getHeroContainer(canvas);
    const stableHeight = getStableHeroHeight();
    const lockWidth = heroContainer?.clientWidth
      ? Math.round(heroContainer.clientWidth)
      : viewport.width;
    const lockHeight =
      stableHeight ||
      (heroContainer?.clientHeight
        ? Math.round(heroContainer.clientHeight)
        : viewport.height);
    engine._heroLockedSize = {
      width: Math.max(lockWidth, 1),
      height: Math.max(lockHeight, 1),
    };
    engine.handleResize();
  }
  return engine;
...
export { Engine, createHeroEngine, initVanillaApp };
