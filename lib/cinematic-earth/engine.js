...
async function createEarth(
  ...
  const [colorMapTex, specularMapTex, normalMapTex, cloudsMapTex, nightMapTex] =
    await Promise.all([
...
function configureHeroDefaults() {
  ...
  // Restore original 8K model fidelity from the source files.
  CONSTANTS.SEGMENTS = use2k ? 64 : 256;
  CONSTANTS.GUI.DEBUG.RESOLUTION_SCALE = getInitialResolutionScale();
  CONSTANTS.GUI.BLOOM.ENABLED = true;
  CONSTANTS.GUI.CHROMATIC_ABERRATION.ENABLED = true;
...
async function createHeroEngine(canvas, { onProgress } = {}) {
